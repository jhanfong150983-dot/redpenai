              ...
              <div className="w-56 border-r border-gray-200 flex flex-col shrink-0 min-h-0">
                ...
                <div className="flex-1 overflow-y-auto py-1">
                  {editingKey.questions.map((q, idx) => {
                    ...
                    // table_cell：答案存在 cells[].answer，每 cell 至少 1 個非空才算有答案
                    const isTableCell = q.questionCategory === 'table_cell'
                    const tableCellHasAnswer = isTableCell && Array.isArray(q.cells)
                      && q.cells.some((c) => {
                        const ca = (c?.answer ?? '').trim()
                        return ca && !PLACEHOLDER_ANSWERS.includes(ca)
                      })
                    // fill_blank 合題：答案存在 parts[].answer，每空都要有非空 answer
                    ...
                    const answerMissing = isTableCell
                      ? !tableCellHasAnswer
                      : hasParts
                        ? !partsAllFilled
