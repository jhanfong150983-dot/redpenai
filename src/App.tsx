...
// 2026-05-19: 8s → 120s。資料量大的老師（佳軒 1040 submissions）first sync 常 10-30s、
// 慢網路會到 60-90s。120s 是「程式真的死了」的 failsafe、不是「應該等多久」。
// 即使 timeout 觸發 retry UI、sync 後續 fire success 仍會自動 settleSuccess 進入。
const INITIAL_SYNC_TIMEOUT_MS = 120_000
...
type PublicPageKey = 'tutorials' | 'school' | 'pricing' | 'faq' | 'contact'
...
const resolvePublicPage = (): PublicPageKey | null => {
  ...
  return PUBLIC_PAGE_PATHS[path] ?? null
}
...
function App() {
  ...
  useEffect(() => {
    ...
    const timeoutId = window.setTimeout(() => {
      ...
      setInitialSyncError((prev) =>
        prev || '同步等待很久了，可能網路有狀況。可以繼續等、或按「重新抓取資料」重試。'
      )
  ...
  // 2026-09-13 四份政策 modal 抽到 components/LegalModals.tsx（官網頁尾共用同一份內文）
  const legalModals = (
    <LegalModal
      kind={isAiDisclaimerOpen ? 'ai' : isIpDisclaimerOpen ? 'ip' : isTermsOpen ? 'terms' : isPrivacyOpen ? 'privacy' : null}
      onClose={() => { setIsAiDisclaimerOpen(false); setIsIpDisclaimerOpen(false); setIsTermsOpen(false); setIsPrivacyOpen(false) }}
    />
  )
  ...
  // 公開頁（不需登入、也不等驗證結果就能看）：教學中心、學校方案。
  // 必須放在 auth gate 之前，否則未登入訪客會被下面的分支導向 LandingPage。
  const publicPage = resolvePublicPage()
  if (publicPage) {
    return (
      <Suspense fallback={
        ...
        {publicPage === 'school' ? <SchoolPlanPage /> : publicPage === 'pricing' ? <PricingPage /> : publicPage === 'faq' ? <FaqPage /> : publicPage === 'contact' ? <ContactPage /> : <TutorialsPage />}
      </Suspense>
    )
  }
