...
- 若圖片中沒有任何方框被標記 → answer 填「?」，提示需人工確認
...
export async function extractAnswerKeyFromImages(
  ...
  const pageIdRule = needsPagePrefix
    ? `\n- 【題目 ID 規則（多頁模式）⚠️關鍵】ID 第一段必須是「照片頁碼」（第幾張照片），絕對不可使用答案卷上印刷的題號數字。\n  格式：<照片頁碼>-<此頁第幾大題>-<第幾小題>-...\n  • <照片頁碼>：當前照片的編號（1, 2, 3...），固定不變\n  • <此頁第幾大題>：此頁第1個大題=1，第2個大題=2，依此類推（無論答案卷印刷的是「一」還是「十一」，只看在這頁是第幾個大題）\n  • <第幾小題>：此大題第1小題=1，第2小題=2，依此類推\n  ❌ 錯誤範例（第2頁，大題印刷號是十=10）：使用印刷號 → "10-1"、"10-2-1"（這是bug，十≠頁碼2）\n  ✅ 正確範例（第2頁，第1個大題無小題）→ "2-1"；第2個大題第1小題 → "2-2-1"；第3個大題第2小題 → "2-3-2"`
    : ''
