...
➊ 單一空格容多種說法（同義詞、近義詞、造詞、**詞義注釋/解釋單一詞語的意思**）→ fill_variants
...
➊ 文字
   └─ 自由說明（解釋現象/原因、舉例、論述）→ short_answer
      ⚠️ 「解釋**單一詞語的意思**」（注釋/詞義題，答案短且固定、容多種說法）→ 走 fill_variants（Bucket B ➊），不是 short_answer。
         此處「解釋」專指解釋現象/原因/概念的論述，不含詞義注釋。
...
▸ fill_variants 「多元填空題」
  ...
  動作：學生填一個值，可有多種接受答案
  欄位：
    · referenceAnswer =「簡易判準」：用一句話寫出「什麼樣的答案算對」的可接受條件，供批改時判斷清單外的答案。
        例：造詞題→「凡含『明』的合法雙字詞皆可（明天/光明/明亮…）」；
            注釋/詞義題→「與『無不、都是』同義者皆可（皆是/全都是/全部…）」。
        ⚠ 不要只填一個範例詞，要寫成「條件」——老師才好改一句話、批改才判得準。
    · acceptableAnswers = 卷面看得到的明確可接受答案（快速命中用；沒有就給空陣列）。
