...

{
  "questions": [{
    ...
    "maxScore": 5,                      // 滿分
    "answerBbox": {"x": 0.1, "y": 0.2, "w": 0.3, "h": 0.05}, // 答案區塊位置（歸一化，0-1；x/y=左上角，w/h=寬高）
...
export async function extractAnswerKeyFromImages(
  ...
  const pageIdRule = needsPagePrefix
    ? `\n- 【題目 ID 規則（多頁模式）⚠️關鍵】ID 第一段必須是「第幾張照片」的編號，絕對不可使用答案卷上印刷的任何數字（包括印刷頁碼、大題號）。\n  格式：<照片序號>-<此頁第幾大題>-<第幾小題>-...\n  • <照片序號>：即這張照片是本次上傳的第幾張（1, 2, 3...），與答案卷紙張上印刷的頁碼無關\n  • <此頁第幾大題>：此頁第1個大題=1，第2個大題=2，依此類推\n  • <第幾小題>：此大題第1小題=1，第2小題=2，依此類推\n  ❌ 錯誤（第2張照片，但答案卷上印有頁碼10）：讀到頁碼10 → "10-1"（bug：10是印刷頁碼，不是照片序號）\n  ✅ 正確（第2張照片，不管印刷頁碼是幾）→ 第1個大題 "2-1"；第2個大題第1小題 "2-2-1"；第3個大題第2小題 "2-3-2"`
    : ''
