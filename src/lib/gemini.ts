...
🆕 不論單空或合題（parts 多空），bbox **一律包整段題幹 + 全部空格標記**。

【涵蓋範圍】
- 從題號（如「1.」「2.」「(1)」）開始 → 框到該題最末字（含單位、句末標點）
- 整段題目（含敘述文字、印刷括號 / 底線 / 方框、紅字答案、單位、標點）必須全部入框
- 兩空之間的印刷字（in / the / is / 是 / 的 / 公尺）**全部在 bbox 內**（與舊規則相反）
...
【⛔ 不要做】
- ⛔ 不要只框 tiny ( ) 或單一空格 — 那是舊規則
- ⛔ 不要為了避開「元 / 公分」單位字而把 bbox 縮小 — 單位字現在要包進來
- ⛔ 不要因為「合題」就拆成多個 bbox — 合題（parts）只有一個 bbox 包整段
