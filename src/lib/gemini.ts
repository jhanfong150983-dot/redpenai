...
- tablePosition（表格內的答案格專用，補充欄位）：
  ...
  - col: 欄位序號（1-based），從表格最左欄開始計數（含列標題欄和已知值欄）
  - row: 列序號（1-based），從表格最上列開始計數（含欄標題列）
  - totalCols: 表格總欄數（含所有欄，包括標題欄和已知值欄）
  - totalRows: 表格總列數
  - colspan: 若此格橫跨多欄，填實際跨欄數（預設 1，可省略）
  - rowspan: 若此格縱跨多列，填實際跨列數（預設 1，可省略）
  - 範例：一個 8 欄 × 3 列的表格，第 1 列是欄標題（就讀國中、光武國中、建功國中…），第 1 欄是列標題（人數、比率）。光武國中的比率「2/5」是黑色印刷 → 不建題。建功國中的比率「3/10」是紅色 → 建題，tablePosition: col=4, row=3, totalCols=8, totalRows=3
