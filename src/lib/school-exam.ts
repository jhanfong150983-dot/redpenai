...
// Step 6 之後的考卷考卷(assignment)建立時會沿用同一資料夾標記,一併在此過濾。
...
// 2026-08-01（user 回報教師端首頁「考卷總覽」跑出一堆行政端的「行政英語TEST」、且顯示「未知班級」）：
//   原本只過濾 classrooms、忘了 assignments——跨班彙總的頁面（首頁總覽、考卷列表）直接讀
//   db.assignments.toArray() 就把學校考卷一起撈進來；班級已被濾掉所以查無名稱＝「未知班級」。
//   以班級為單位進入的頁面不受影響（班級沒出現、考卷自然進不去）。
...
/** 過濾掉學校考卷的考卷：①所屬班級是學校考卷班 ②考卷本身標了學校考卷資料夾（兩條件皆擋、防單邊漏標） */
