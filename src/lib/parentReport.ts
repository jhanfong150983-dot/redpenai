...
import { ALL_MATH_NODES, GUOYU_NODES, SOCIAL_NODES, ALL_SCIENCE_NODES } from '@/data/curriculumNodes'
...
只輸出 JSON：{"items":[{"questionId":"...","topic":"...","knowledgePoints":[...],"ability":"...","note":"..."}]}`
  ...
  //   自然：國中生物等有節點；小學自然/無節點碼→第二層自動跳過(只走第一層)
  const isScience = /自然|生物|理化|物理|化學|地球科學|地科/.test(subj)
  if (isMath || isGuoyu || isSocial || isScience) {
    // 按第一層 code 分組（只處理表內有節點的 code）——各科各用自己的節點池(代碼不衝突)
    const pool = isMath ? ALL_MATH_NODES : isGuoyu ? GUOYU_NODES : isSocial ? SOCIAL_NODES : ALL_SCIENCE_NODES
