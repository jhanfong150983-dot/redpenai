...
import { useState, useEffect, useCallback, useMemo, useRef, type WheelEvent as ReactWheelEvent, type PointerEvent as ReactPointerEvent } from 'react'
...
export type CardStage =
  ...
  // ⚠ 2026-08-08：'pending_review' 已是孤兒——deriveCardStage 從來不回傳它（見該函式內註解），
  //   所以 stageMap.pending_review / needReview 桶恆為空。留著只為不動一鍵批改的桶簽名，
  //   別再為它寫邏輯（我曾誤以為它活著而白改了三個消費端）。
  | 'pending_review'   // （孤兒）待複核
...
export function deriveCardStage(sub: Submission | undefined, correctionStatus?: string): CardStage {
  ...
  if (isExtracted) {
    ...
    // 2026-08-08 修顯示錯誤：原本「還有待確認題」→ 回 'not_extracted'（顯示「未擷取」），
    //   但這些卷明明已擷取過（有 arbiterDecisions / details），顯示未擷取會誤導老師去重新截取。
    //   0 審查架構下「待確認題」已不存在於新資料；全庫實測仍有 38 份這種舊卷
    //   （數習70-73 13 份、iVocab L4U46-50 23 份、B2L9習作簿 2 份）→ 一律顯示「待批改」才對，
    //   而且按「智慧批改」就能處理（與待複核閘門的訊息指向同一條出路）。
...
// 2026-08-08 移除 'awaiting_review'（0 審查架構下已無任何 setter，見人工審查 UI 移除註解）
type GradingPhase = 'idle' | 'phase_a_running' | 'phase_b_running' | 'report_running'
...
// ─── 2026-08-08 人工審查 UI 整段移除（0 審查架構）────────────────────────────
//   applyEscalationChain + applyZeroReviewTail 都預設開 → 每一格都轉成 arbitrated_agree，
//   程式中已無任何 setGradingPhase('awaiting_review')（四個編排分支於階段 3a 移除）→
//   BatchConsistencyReviewSection / ConsistencyQuestionCard 與其 render site 成為孤島。
//   實測依據：全庫 904 份有 phase_a_state 的卷，鏈上線（2026-07-11）後批改的卷
//   一份都沒有殘留 needs_review。⚠ 代價：ESCALATION_CHAIN=0 / ZERO_REVIEW_TAIL=0
//   兩個 kill switch 變成單向（關掉後 needs_review 格沒有介面可處理）。user 拍板。
//   OriginalPageViewer 保留——學生端「看原圖」仍在用。
...
export default function GradingPage({
  ...
  // 2026-08-01: 題目詳情可編輯狀態初始化已隨 modal 搬進 SubmissionDetailModal
    ...
    <div className={`${embedded ? 'bg-white p-0' : 'min-h-screen bg-white p-4'}`}>
      ...
      {selectedSubmission && (
        <SubmissionDetailModal
          /* key=卷 id:換學生時強制重新掛載 → 編輯狀態/放大鏡自動重置
             (取代原本散在 GradingPage 的 setPreviewLensActive(false) effect) */
          key={selectedSubmission.submission.id}
          submission={selectedSubmission.submission}
          student={selectedSubmission.student}
          assignment={assignment ?? undefined}
          classroomName={classroom?.name}
          isBusy={isBusy}
          isStale={isPhaseAStale(selectedSubmission.submission, correctionStatusByStudent[selectedSubmission.submission.studentId])}
          onClose={handleCloseModal}
          onUpdated={(updated) => {
            setSubmissions((prev) => new Map(prev).set(updated.studentId, updated))
            setSelectedSubmission((prev) => (prev ? { ...prev, submission: updated } : prev))
          }}
        />
      )}
    </div>
